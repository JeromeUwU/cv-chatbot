...
MAX_QUESTIONS = 10  # 🔒 limite par session

# ------------------------
# 🔐 Gate "on-topic" (léger, local)
# ------------------------
# 🔓 Thèmes généraux d'entretien (FR/EN)

TECH_WHITELIST = {
    # thèmes généraux entretien
...
# 🔓 Compétences “capability” génériques liées au poste (FR/EN)
...
# 🔎 Motifs soft-skills faciles (regex “contient”)
SOFT_SKILL_PATTERNS = [
...
# ------------------------
# 🧠 System prompt (inclut un garde-fou supplémentaire côté modèle)
# ------------------------
...
if "messages" not in st.session_state:
    ...
    st.session_state.blocked = False  # une fois à True, on ne débloque pas

# Rebuild prompt si style change
if st.session_state.get("sys_style") != sys_style:
    # Ne pas reset l'historique, mais MAJ du system pour réponses futures
...
# ------------------------
# 🚦 Gate & exécution (double garde avant appel Groq)
# ------------------------
if prompt:
    # Re-check serveur : si déjà à la limite → bloquer *sans* appeler Groq
    if user_count() >= MAX_QUESTIONS or st.session_state.blocked:
        st.session_state.blocked = True
        with st.chat_message("assistant"):
            st.error("Limite atteinte. Le chat est verrouillé pour cette session.")
    else:
        # gate “on-topic” (permissif)
        if not is_on_topic(prompt):
            ...
            # Ajoute la question puis stream (appel Groq)
            ...
            # Mise à jour du verrou après réponse
